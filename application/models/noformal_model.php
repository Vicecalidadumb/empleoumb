...
class Noformal_model extends CI_Model {

    public function get_documents_noformal_user($INSCRIPCION_PIN, $STATE = 1) {
        $where = " AND DOCUMENTO_ESTADO=$STATE ";
        if ($STATE == 'ALL') {
            $where = "";
        }        
        $SQL_string = "SELECT *
                      FROM {$this->db->dbprefix('documento')} d
                      WHERE INSCRIPCION_PIN = '{$INSCRIPCION_PIN}' $where AND TIPO_DOCUMENTO_ID=3";
