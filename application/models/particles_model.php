...
class Particles_model extends CI_Model {

    public function get_documents_particles_user($INSCRIPCION_PIN, $STATE = 1) {
        ...
        $SQL_string = "SELECT *
                      ...
                      WHERE INSCRIPCION_PIN = '{$INSCRIPCION_PIN}' $where AND TIPO_DOCUMENTO_ID>5";
        $SQL_string_query = $this->db->query($SQL_string);
        return $SQL_string_query->result();
    }

    public function get_documents_user($INSCRIPCION_PIN) {
        $SQL_string = "SELECT *
                      FROM {$this->db->dbprefix('documento')}
                      WHERE INSCRIPCION_PIN = '{$INSCRIPCION_PIN}' AND DOCUMENTO_ESTADO=1";
        $SQL_string_query = $this->db->query($SQL_string);
        return $SQL_string_query->result();
    }
