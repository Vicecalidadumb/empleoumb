...
class Noformal extends CI_Controller {
    ...
    public function index() {
        $data['title'] = 'Aplicativo de Cargue de Documentos - Educacion No Formal';
        $data['documents_noformal_user'] = $this->noformal_model->get_documents_noformal_user($this->session->userdata("INSCRIPCION_PIN"));
        $data['content'] = 'noformal/add';

        $data['modalidades'] = get_dropdown($this->noformal_model->get_modalidades(), 'MODALIDAD_ID', 'MODALIDAD_NOMBRE');
    ...
    public function insert() {
        ...
        $this->form_validation->set_rules('UNIVERSIDAD', 'Instituci&oacute;n', 'required|trim');
        $this->form_validation->set_rules('TITULO', 'Nombre del Curso', 'required|min_length[2]|trim');
        $this->form_validation->set_rules('INTENSIDAD', 'Intensidad Horaria', 'numeric|required|trim');
        $this->form_validation->set_rules('FECHA_TERMINACION', 'Fecha de Terminaci&oacute;n', 'required|trim');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!$this->upload->do_upload($field_name)) {
                ...
                $data = array(
                    ...
                    'INTENSIDAD' => addslashes($this->input->post('INTENSIDAD', TRUE)),
                );

                $insert = $this->noformal_model->insert_document($data);
                if ($insert) {
    ...
    public function view_document($DOCUMENTO_ID) {
        ...
        $document = $this->noformal_model->get_document_user($this->session->userdata("INSCRIPCION_PIN"), $DOCUMENTO_ID);
    ...
    public function delete_document($DOCUMENTO_ID) {
        $DOCUMENTO_ID = deencrypt_id($DOCUMENTO_ID);
        $this->noformal_model->delete_document_user($this->session->userdata("INSCRIPCION_PIN"), $DOCUMENTO_ID);
